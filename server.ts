...
app.get('/api/tools/windows-installer-script', (req: Request, res: Response) => {
  ...
  try {
    ...
    if (fs.existsSync(icoPath)) {
      icoBase64 = fs.readFileSync(icoPath).toString('base64');
    }
  } catch {}
  ...
  const ps1Lines = [
    ...
    `$AppUrl = "${fullAppUrl}"`,
    '',
    'Write-Host "[1/5] Initializing Abel AI Application Directories..." -ForegroundColor Cyan',
    ...
    '',
    'Write-Host "[2/5] Writing Official High-Resolution Abel AI Desktop Icon..." -ForegroundColor Cyan',
    icoBase64
      ? `try { [System.IO.File]::WriteAllBytes($IconPath, [System.Convert]::FromBase64String("${icoBase64}")) } catch {}`
      : `try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri "$AppUrl/abel_icon.ico" -OutFile "$IconPath" -UseBasicParsing -TimeoutSec 10 } catch {}`,
    '',
    'Write-Host "[3/5] Locating Chrome/Edge standalone app runtime..." -ForegroundColor Cyan',
    ...
    '',
    'Write-Host "[4/5] Creating Desktop & Start Menu Shortcuts with custom Gold Icon..." -ForegroundColor Cyan',
    ...
    '$Shortcut.Save()',
    '',
    '$SmShortcut = $WshShell.CreateShortcut("$StartMenuDir\\Abel AI.lnk")',
    ...
    '',
    'Write-Host "[5/5] Launching Abel AI Desktop App..." -ForegroundColor Green',
    'Start-Process $targetExe $appArgs',
    '',
    'Write-Host "=================================================================" -ForegroundColor Yellow',
    'Write-Host " Abel AI is now installed on your Windows machine!" -ForegroundColor Green',
    'Write-Host " - Desktop Icon: Created on Desktop (Gold Abel Medallion)" -ForegroundColor Cyan',
    'Write-Host " - Start Menu:   Created in Start Menu -> Abel AI" -ForegroundColor Cyan',
    'Write-Host " - Native App:   Running frameless standalone window" -ForegroundColor Cyan',
    'Write-Host "=================================================================" -ForegroundColor Yellow',
  ];
...
app.get('/api/tools/windows-batch-installer', (req: Request, res: Response) => {
  ...
  const chunks: string[] = [];
  for (let i = 0; i < icoBase64.length; i += 64) {
    chunks.push(icoBase64.substring(i, i + 64));
  }
  ...
  const batLines = [
    ...
    `set "APP_URL=${fullAppUrl}"`,
    'if not "%~1"=="" set "APP_URL=%~1"',
    'if not exist "%APP_DIR%" mkdir "%APP_DIR%"',
    'if not exist "%START_DIR%" mkdir "%START_DIR%"',
    'echo.',
    'echo [*] Extracting embedded Gold Abel AI Desktop Icon...',
    '(',
    ...
    'echo  - Custom Gold Medallion Icon active',
    'echo ===============================================================================',
    'timeout /t 3 >nul',
    'exit',
  ];
