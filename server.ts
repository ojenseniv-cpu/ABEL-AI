...
app.get('/api/tools/windows-installer-script', (req: Request, res: Response) => {
  ...
  const ps1Lines = [
    ...
    '}',
    '',
    'Write-Host "[2/5] Downloading Official Gold Abel AI Desktop Icon..." -ForegroundColor Cyan',
    'try {',
    '    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12',
    `    Invoke-WebRequest -Uri "$AppUrl/abel_icon.svg" -OutFile "$InstallDir\\abel_icon.svg" -UseBasicParsing`,
    '    Copy-Item "$InstallDir\\abel_icon.svg" "$IconPath" -Force',
    '} catch {',
    '    Write-Host "Using default system icon" -ForegroundColor DarkGray',
    '}',
    '',
...
app.get('/api/tools/windows-batch-installer', (req: Request, res: Response) => {
  ...
  const batLines = [
    ...
    'echo.',
    'echo [2/4] Downloading Gold Abel AI Desktop Icon...',
    `powershell -NoProfile -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; try { Invoke-WebRequest -Uri '${fullAppUrl}/abel_icon.svg' -OutFile '%LOCALAPPDATA%\\AbelAI\\abel_icon.svg' -UseBasicParsing; Copy-Item '%LOCALAPPDATA%\\AbelAI\\abel_icon.svg' '%LOCALAPPDATA%\\AbelAI\\abel_icon.ico' -Force } catch {}"`,
    'echo.',
